...
def huggingface_chat(messages):
    prompt = '\n'.join(f"{m.get('role', 'user')}: {m.get('content', '')}" for m in messages)[-3000:]
    payload = json.dumps({'inputs': prompt, 'options': {'wait_for_model': True}}).encode('utf-8')
...
def local_chat(messages):
    task = next((m.get('content', '') for m in reversed(messages) if m.get('role') == 'user'), 'No task')
    return '\n'.join([
        'Local deterministic planner',
        f'Task: {task[:300]}',
        '1) Define scope and success criteria',
        '2) Build staged execution plan',
        '3) Validate quality gates',
        '4) Prepare deployment and rollback',
    ])
...
def provider_status():
    ...
    return {
        'ok': True,
        'providers': [
            {'id': 'ollama-local', 'label': 'Ollama Local (offline)', 'available': ollama_ok},
            {'id': 'hf-free', 'label': 'HuggingFace Inference (free/online)', 'available': True},
            {'id': 'local-sim', 'label': 'Local deterministic fallback', 'available': True},
        ],
...
def list_agent_library():
    ...
    for file in sorted(AGENT_LIBRARY_DIR.glob('*.json')):
        try:
            ...
            data = {}
        items.append({'file': file.name, 'name': data.get('name') or file.stem, 'source': data.get('_source', 'local'), 'size': file.stat().st_size, 'downloadUrl': f'/agent_library/{file.name}', 'content': data})
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/api/ollama/status':
            ...
            self._json(200, provider_status())
            return
    ...
    def do_POST(self):
        ...
        if self.path == '/api/agents/save':
            ...
            try:
                self._json(200, save_agent(payload.get('agent')))
            except Exception as err:  # noqa: BLE001
                self._json(200, {'ok': False, 'error': str(err)})
            return
